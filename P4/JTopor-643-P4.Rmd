...
most_fans <- subset(lfm_art, id %in% top_400$artistID)$name

most_ids <- subset(lfm_art, id %in% top_400$artistID)$id

missing <- subset(top_400, !(artistID %in% most_ids))
...
```{r, message = FALSE, warning = FALSE}
kable(head(cbind(most_fans, top_374$TotalUsers), 20), col.names = c("Artist", "Listeners"))
```
...
for (i in 1:length(b_pred@items)){
  ...
  # if to few recommendations generated, sample 10 at random from the top374
  if(num_newrecs < 10) {
...
urecs <- subset(user_tenrecs, userID == user)[2:11]
